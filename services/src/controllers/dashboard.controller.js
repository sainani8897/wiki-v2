...
exports.index = async function (req, res, next) {
  try {
    ...
    if (dashboard.totalDocs > 0) { return res.send({ status: 200, message: 'Data found', data: dashboard }) } else {
      return res.send({
        status: 204,
...
exports.show = async function (req, res, next) {
  ...
  try {
    const invoice = await Payment.findById({ _id })
    if (invoice) { return res.send({ status: 200, message: 'Data found', data: invoice }) } else throw new NotFoundException('No Data Found!')
  } catch (error) {
    next(error)
  }
}
...
exports.update = async function (req, res, next) {
  try {
    ...
    const order = await Payment.findById({ _id })
