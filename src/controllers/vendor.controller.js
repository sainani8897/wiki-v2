...
exports.index = async function (req, res, next) {
  try {
    ...
    if (vendors.totalDocs > 0) { return res.send({ status: 200, message: 'Data found', data: vendors }) } else {
      return res.send({
        status: 204,
...
exports.show = async function (req, res, next) {
  ...
  try {
    ...
    if (vendor) { return res.send({ status: 200, message: 'Data found', data: vendor }) } else throw new NotFoundException('No Data Found!')
  } catch (error) {
    next(error)
  }
}
...
exports.update = async function (req, res, next) {
  try {
    ...
    const vendor = await Vendor.findById({ _id })
    if (!vendor) { return res.send({ status: 404, message: 'No data found', data: {} }) }
